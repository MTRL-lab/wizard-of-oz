...
const CoDesignMeetingTask = ({ task,t }) => {
    ...
    const subject = t('Project Summary Meeting')
    const summary = t('final_meeting_summary')
    const start = new Date('July 31, 2022 17:00 GMT+03:00')
    const end = new Date('July 31, 2022 20:00 GMT+03:00')
    const meeting = {
        start,
        end,
        summary:subject,
    ...
    const confirm = () => {
        ...
        newMeeting.description = meeting.description.join('\n')
        return api
    ...
    return <Card>
        ...
        <CardContent>
            <Text>
                {meeting.description.map((part,index)=><p key={index}>{part}</p>)}
            </Text>
...
export default withTranslation()(CoDesignMeetingTask);
