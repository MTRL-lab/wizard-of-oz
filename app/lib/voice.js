...
const support = {
  "en-US": {
    ...
    stt: "en-US",
    ttsName: {
      architect: "en-US-Wavenet-C",
      designer: "en-US-Wavenet-D",
    },
  },
  "en-UK": {
    tts: "en-UK",
    stt: "en-UK",
    ttsName: {
      architect: "en-GB-Wavenet-A",
      designer: "en-GB-Wavenet-B",
    },
  },
  // 'de-DE': {
  //     'tts': 'de-DE',
...
export const langNameTts = (language, user) => {
  return support[language]["ttsName"][user];
};

export const textToSpeech = (text, languageCode, user) => {
  ...
  const sha = sha1(`${languageCode}_${text}_${user}`);
  ...
  try {
    const request = {
      ...
      voice: {
        ...
        // ssmlGender,
        name: langNameTts(languageCode, user),
      },
